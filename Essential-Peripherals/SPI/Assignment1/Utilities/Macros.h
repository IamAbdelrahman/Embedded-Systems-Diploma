 ...
 * in microcontroller applications for bit manipulation and other basic operations.
...
/**
 * @brief Writes a value to specific bit in a register.
 * @param REG The register to modify.
 * @param VALUE The value to set the nibble to.
 * @param POSITION The position of the nibble to modify.
 */
#define WRITE_BIT(REG, VALUE, POSITION)					(REG = (REG & ~(0b1U << POSITION)) | \
		((VALUE & 0b1) << POSITION))
