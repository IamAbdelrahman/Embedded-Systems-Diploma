...
ST_Queue_t Q;
ST_UARTIrqSrc_t uartIrqSrc;
void (*gPf_IRQ_Callback_Usart[3])(ST_UARTIrqSrc_t) = {NULL, NULL, NULL};
typedef struct {
...
ST_Flags_t Flag = {1U, 1U, 1U, 1U, 0U};
...
void USART1_IRQHandler(void) {
	uartIrqSrc.RXNE = READ_BIT(*Status[UART_1], 5);
	uartIrqSrc.TC = READ_BIT(*Status[UART_1], 6);
	uartIrqSrc.TXE = READ_BIT(*Status[UART_1], 7);
	gPf_IRQ_Callback_Usart[0](uartIrqSrc);
}

void USART2_IRQHandler(void) {
	uartIrqSrc.RXNE = READ_BIT(*Status[UART_2], 5);
	uartIrqSrc.TC = READ_BIT(*Status[UART_2], 6);
	uartIrqSrc.TXE = READ_BIT(*Status[UART_2], 7);
	gPf_IRQ_Callback_Usart[1](uartIrqSrc);
}

void USART3_IRQHandler(void) {
	uartIrqSrc.RXNE = READ_BIT(*Status[UART_3], 5);
	uartIrqSrc.TC = READ_BIT(*Status[UART_3], 6);
	uartIrqSrc.TXE = READ_BIT(*Status[UART_3], 7);
	gPf_IRQ_Callback_Usart[2](uartIrqSrc);
}
...
void UART_Init(ST_UARTConfig_t *const configPtr) {
  vuint8_t i = 0;
  Flag.uartNo = configPtr->usartNumber;
  for (i = 0; i < configSizeUsart; i++) {
    /*- Set the UART pins */
    GPIO_Init(configPtr[i].pSetTxPin, 1);
    GPIO_Init(configPtr[i].pSetRxPin, 1);
...
void UART_SendString(vuint8_t *pStr) {
    while (*pStr > 0) {
        UART_SendData(Flag.uartNo, pStr++);
    }
    vuint8_t defaultStop = TERMINATOR;
    UART_SendData(Flag.uartNo, &defaultStop);
}

/*
 * TODO
 * No-Blocking Mechanism and it divides into static and dynamic design
 * a 2nd approach is using the time-out handling
 */
...
void UART_ReceiveString(vuint8_t *pStr) {
    ...
    do {
        UART_ReceiveData(Flag.uartNo, &data);
        Enqueue(data, &Q);
...
void UART_SendIntNum(vuint32_t Num) {
    ...
    for (i = 0; i < 4; i++) {
        UART_SendData(Flag.uartNo, pByt++);
    }
}
...
void UART_ReceiveIntNum(vuint32_t *pNum) {
    ...
    pByt = (vuint8_t*) pNum;
    for (i = 0; i < 4; i++)
        UART_ReceiveData(Flag.uartNo, pByt++);
}
